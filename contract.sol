...
contract Token is Context, IERC20, Ownable {
    ...
    address public router = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //pancake swap 
    ...
    constructor () payable {
        _name = "Moon Shiba Doge Floki Lambo Baby";
        _symbol = "MSDFLB";
        _decimals = 12;
        _tTotal = 4 * 10**15 * 10**12; // 4 quadrillion
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(0x0000000000000000000000000000000000000000); // Marketing and development address - will be set after deployment
        feeWalletCharity = payable(0x0000000000000000000000000000000000000000); // Charity address used for donations - will be set after deployment
        walletCharityFeeInBNB = true;
        
        rewardToken = 0x0000000000000000000000000000000000000000; //no reward token set, reward tax set to 0. 
        minimumTokenBalanceForDividends = 1 * 10**6 * 10**12; // 1 million

        _maxTxAmount = 2 * 10**13 * 10**12; // 20 trillion or 0.5% of total supply
        _maxWalletAmount = 4 * 10**13 * 10**12; // 40 trillion or 1% of total supply
        ...
        //set the various availible fees
        _taxFee = 0; // tax fee
        _liquidityFee = 5; // automatic liquidity generation tax
        _burnFee = 0; // automatic burn tax
        _buybackFee = 0; // automatic buy back and burn tax
        _walletFee = 5; // marketing and development tax
        _walletCharityFee = 0; // charity tax
        _rewardFee = 0; // reward tax
    ...
    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner() {
        require(maxWalletPercent >= minMxWalletPercentage && maxWalletPercent <=10000,"err");
    ...
    function recoverBEP20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        // do not allow recovering self token, reward token, or LP tokens
        require(tokenAddress != address(this), "Cannot withdraw native tokens");
        require(tokenAddress != rewardToken, "Cannot withdraw reward tokens");
        require(tokenAddress != pcsV2Pair, "Can't transfer out LP tokens!");
